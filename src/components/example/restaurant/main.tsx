...
import { currencyFormatter, getCategoryAPI, getURLBaseBackend } from '@/src/utils/api';
...
const RMain = (props: IProps) => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            ...
            if (res.data) {
                const response = res.data.map((item: ICategories, index) => {
                    return {
                        ...item,
                        key: item.id,
                        title: item.name,
    ...
    return (
        <View>
            ...
            <View style={styles.header}>
                <Image
                    source={{ uri: `${getURLBaseBackend()}/storage/book/${book?.thumbnail}` }}
            ...
            <Animated.View style={[animatedInfoStyle]}>
                <Info infoHeight={INFO_HEIGHT} book={book} />
            </Animated.View>
                    ...
                    return (
                        <View style={{
                            ...
                            <View style={{ flex: 1, gap: 10 }}>
                                <View><Text>{book.mainText}</Text></View>
                                <View><Text>{book.author}</Text></View>
